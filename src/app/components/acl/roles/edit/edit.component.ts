...
export class EditComponent implements OnInit, OnDestroy {
  ...
  constructor(private notification: NotificationService, private activeRouter: ActivatedRoute, private route: Router) {
    this.navigationSubscription = this.route.events.subscribe((e: any) => {
      if (e instanceof NavigationEnd) {
        ...
        store.dispatch({ type: FETCH_ROLE_DETAIL_REQUESTED, data: roleId, com: ROLE_COMP });
      }
    });
  }
  ...
  selectAllPermissionGroup(group) {
    store.dispatch({ type: SELECT_ALL_PERMISSIONS_GROUP, data: group, com: ROLE_COMP });
  }

  savePermission() {
    let permissions = (store as any).getState().Acl.Roles.roleDetail.groups;
