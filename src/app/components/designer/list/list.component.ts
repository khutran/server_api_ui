import { DELETE_DESIGNER_REQUESTED } from './../edit/edit.actions';
import { Component, OnInit, OnDestroy } from '@angular/core';
import store from './../../../store/store.module';
import { FETCH_DESIGNERS_REQUESTED, SORT_DESIGNERS_REQUESTED } from './list.actions';
...
export class ListComponent implements OnInit, OnDestroy {
  ...
  constructor(private notification: NotificationService, private activeRouter: ActivatedRoute, router: Router) {
    ...
    this.navigationSubscription = this.router.events.subscribe((e: any) => {
      if (e instanceof NavigationEnd) {
        store.dispatch({ type: FETCH_DESIGNERS_REQUESTED, data: this.getQuery() });
      }
    });
  }

  ngOnInit() {
    store.dispatch({ type: FETCH_DESIGNERS_REQUESTED });
  }
  ...
  deleteItem(id) {
    if (confirm('Do you want to delete this designer?')) {
      store.dispatch({ type: DELETE_DESIGNER_REQUESTED, data: id });
    }
  }
  ...
  sortBy(field) {
    ...
    store.dispatch({ type: SORT_DESIGNERS_REQUESTED, data: this.orders });
  }
