...
function* allSaga() {
  ...
  if (!fetchStatus) {
    try {
      ...
      yield put({type: GET_ALL_STATUSS_SUCCEEDED, data: results});
    } catch (e) {
      yield put({ type: API_CALL_ERROR, error: e });
    }
  } else {
    const data = yield select((state) => (state as any).Status.all.items);
    yield put({type: GET_ALL_STATUSS_SUCCEEDED, data: data});
  }
}

function* watchFetchAllStatussRequest() {
  yield takeEvery(GET_ALL_STATUSS_REQUESTED, allSaga);
}
export default _.map([...listSaga, ...editSaga, ...createSaga, watchFetchAllStatussRequest], item =>
  fork(item)
);
