...
import { EDIT_STATUS_REQUESTED, RENDER_EDIT_STATUS_FORM_REQUESTED } from './edit.actions';
...
export class EditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.store.dispatch({ type: RENDER_EDIT_STATUS_FORM_REQUESTED, data: { id: this.activatedRoute.snapshot.params.id, inputs: inputs } });
  }
  ...
  onSubmit(form) {
    ...
    if (form.valid) {
      store.dispatch({
        ...
        data: _.assign(form.value, { id: store.getState().Status.edit.item.id })
      });
    }
  }
