import { DELETE_STATUS_REQUESTED } from './../edit/edit.actions';
import { FETCH_STATUSS_REQUESTED, SORT_STATUSS_REQUESTED } from './list.actions';
...
export class ListComponent implements OnInit, OnDestroy {
  ...
  constructor(store: Store, private notification: NotificationService, private activedRoute: ActivatedRoute, router: Router) {
    ...
    this.navigationSubscription = this.router.events.subscribe((e: any) => {
      if (e instanceof NavigationEnd) {
        this.store.dispatch({ type: FETCH_STATUSS_REQUESTED, data: this.parseQueryParams() });
      }
    });
  }

  ngOnInit() {
    // this.store.dispatch({ type: FETCH_STATUSS_REQUESTED });
  }
  ...
  delete(id) {
      this.store.dispatch({ type: DELETE_STATUS_REQUESTED, data: id });
  }
}
