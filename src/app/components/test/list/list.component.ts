import { DELETE_TEST_REQUESTED } from './../edit/edit.actions';
import { FETCH_TESTS_REQUESTED, SORT_TESTS_REQUESTED } from './list.actions';
...
export class ListComponent implements OnInit, OnDestroy {
  ...
  constructor(store: Store, private notification: NotificationService, private activedRoute: ActivatedRoute, router: Router) {
    ...
    this.navigationSubscription = this.router.events.subscribe((e: any) => {
      if (e instanceof NavigationEnd) {
        this.store.dispatch({ type: FETCH_TESTS_REQUESTED, data: this.parseQueryParams() });
      }
    });
  }

  ngOnInit() {
    // this.store.dispatch({ type: FETCH_TESTS_REQUESTED });
  }
  ...
  delete(id) {
      this.store.dispatch({ type: DELETE_TEST_REQUESTED, data: id });
  }
}
