import { DELETE_CURRENCY_REQUESTED } from './../edit/edit.actions';
import { FETCH_CURRENCYS_REQUESTED, SORT_CURRENCYS_REQUESTED } from './list.actions';
...
export class ListComponent implements OnInit, OnDestroy {
  ...
  constructor(private notification: NotificationService, private activeRouter: ActivatedRoute, router: Router) {
    ...
    this.navigationSubscription = this.router.events.subscribe((e: any) => {
      if (e instanceof NavigationEnd) {
        store.dispatch({ type: FETCH_CURRENCYS_REQUESTED, data: this.getQuery() });
      }
    });
  }

  ngOnInit() {
    store.dispatch({ type: FETCH_CURRENCYS_REQUESTED });
  }
  ...
  deleteItem(id) {
    if (confirm('Do you want to delete this currency?')) {
      store.dispatch({ type: DELETE_CURRENCY_REQUESTED, data: id });
    }
  }
}
