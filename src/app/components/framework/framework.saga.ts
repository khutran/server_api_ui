...
function* allSaga() {
  ...
  if (!fetchStatus) {
    try {
      ...
      yield put({type: GET_ALL_FRAMEWORKS_SUCCEEDED, data: results});
    } catch (e) {
      yield put({ type: API_CALL_ERROR, error: e });
    }
  } else {
    const data = yield select((state) => (state as any).Framework.all.items);
    yield put({type: GET_ALL_FRAMEWORKS_SUCCEEDED, data: data});
  }
}
...
export default _.map([...listSaga, ...editSaga, ...createSaga, watchFetchAllFrameworksRequest], item =>
  fork(item)
);
