...
export class ListComponent implements OnInit, OnDestroy {
  ...
  constructor(private notification: NotificationService, private activeRouter: ActivatedRoute, router: Router) {
    ...
    this.navigationSubscription = this.router.events.subscribe((e: any) => {
      if (e instanceof NavigationEnd) {
        store.dispatch({ type: FETCH_<%= underscore(name).toUpperCase() %>S_REQUESTED, data: this.getQuery() });
      }
    });
  }

  ngOnInit() {
    store.dispatch({ type: FETCH_<%= underscore(name).toUpperCase() %>S_REQUESTED });
  }
  ...
  deleteItem(id) {
    if (confirm('Do you want to delete this <%= underscore(name).replace("_", "")  %>?')) {
      store.dispatch({ type: DELETE_<%= underscore(name).toUpperCase() %>_REQUESTED, data: id });
    }
  }
}
