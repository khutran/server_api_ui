    ...
    if (readVal) {
      ...
      if (val) {
        ...
        if (str.indexOf("constructor(") > -1) {
          declarationRecorder.insertRight(str.indexOf("constructor(") + "constructor(".length + 1, `    public ${strings.camelize(options.name)}: ${strings.classify(options.name)}Service,`);
        }
      }
    }
    ...
    if (readVal) {
      ...
      if (val) {
        ...
        if (str.indexOf("const RootReducer = (") > -1) {
          declarationRecorder.insertLeft(str.indexOf("const RootReducer = (") - 2, `import { ${strings.classify(options.name)} } from './../components/${options.name}/${options.name}.reducer';`);
        }
    ...
    if (readVal) {
      ...
      if (val) {
        ...
        if (str.indexOf("function* watchApiCallError() {") > -1) {
          declarationRecorder.insertLeft(str.indexOf("function* watchApiCallError() {") - 2, `import ${strings.camelize(options.name)}Saga from './../components/${options.name}/${options.name}.saga';`);
        }
    ...
    if (readVal) {
      ...
      if (val) {
        ...
        if (str.indexOf("const appRoutes: Routes = [") > -1) {
          declarationRecorder.insertLeft(str.indexOf("const appRoutes: Routes = [") - 2, `import { ${strings.classify(options.name)}Component } from './components/${options.name}/${options.name}.component';`);
        }
        if (str.indexOf("{ path: '', redirectTo: 'inventory', pathMatch: 'full' },") > -1) {
          declarationRecorder.insertRight(str.indexOf("{ path: '', redirectTo: 'inventory', pathMatch: 'full' },") + "{ path: '', redirectTo: 'inventory', pathMatch: 'full' },".length + 1, `      {
        path: '${options.name}',
  ...
  return (host: Tree) => {
    ...
    if (readVal) {
      ...
      if (val) {
        let str = val.toString();
        if (str.indexOf("@NgModule({") > -1) {
          declarationRecorder.insertLeft(str.indexOf("@NgModule({") - 2, `import { ${strings.classify(options.name)}Component } from "./${options.name}/${options.name}.component";`);
          declarationRecorder.insertLeft(str.indexOf("@NgModule({") - 2, `
